...
export default {
    ...
    handleUpload() {
      ...
      axios({
        ...
        .then(function(response) {
          ...
          // 解析JSON字符串
          const raw_data = JSON.parse(response.data['data']['raw_out']);
          raw_data.forEach(item => {
            const coordinates = item[0];
            const description = item[1];
            _this.$data.ocrRaw += "[[" + coordinates + "],\"" + description + "\"]" + '\r'
            _this.$data.ocrText += description + ' '
          });
  ...
  mounted: function() {
    ...
    // this.userID = this.$route.meta.userID || 0; // 获取存储在 meta 中的 userID
